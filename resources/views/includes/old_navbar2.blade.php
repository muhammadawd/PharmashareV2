    ...
    <div class="container">
        ...
        <ul class="navbar-nav ml-auto d-md-flex d-lg-flex  direction">
            ...
            <li class="nav-item dropdown d-none d-md-block">
                <a href="#" class="nav-link dropdown-toggle" id="navbarDropdown" data-toggle="dropdown" style="border-radius:50px;border:1px solid #5c39bb">
                    <i class="now-ui-icons files_paper" aria-hidden="true"></i>
                    <p class="lead">{{app()->getLocale() == 'ar' ? 'القائمة' : 'Store'}}</p>
                </a>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown"
                     style="min-width: 280px">
                    @if(auth()->user()->role_id == 1)
                        <a class="dropdown-item" href="{{route('getAdminAddProductView')}}">
                            <i class="now-ui-icons fas fa-prescription-bottle"></i>
                            <p>{{__('profile.add_drug')}}  </p>
                            <!--<label class="badge badge-info float-right">Admin</label>-->
                        </a>
                        <a class="dropdown-item" href="{{route('getApproveProductsView')}}">
                            <i class="now-ui-icons shopping_box"></i>
                            <p> {{__('profile.approve_drugs')}} </p>
                            <!--<label class="badge badge-info float-right">admin</label>-->
                        </a> 
                        <a class="dropdown-item" href="{{route('getUsersAccounts')}}">
                    ...
                    @if(auth()->user()->role_id == 2)
                        <a class="dropdown-item" href="{{route('getAddProductView')}}">
                            <i class="now-ui-icons shopping_tag-content"></i>
                            <p>   {{__('profile.add_drug_store')}}  </p>
                            <!--<label class="badge badge-danger float-right">Drug Store</label>-->
                        </a>
                        <a class="dropdown-item" href="{{route('getAllProductsView')}}">
                            <i class="now-ui-icons shopping_box"></i>
                            <p>{{__('profile.store')}} </p>
                            <!--<label class="badge badge-danger float-right">Drug Store</label>-->
                        </a> 
                        <a class="dropdown-item" href="{{route('getSalesView')}}">
                            <i class="now-ui-icons users_single-02"></i>
                    ...
                    @if(auth()->user()->role_id == 3)
                        <a class="dropdown-item" href="{{route('getProductsView')}}">
                            <i class="now-ui-icons shopping_cart-simple"></i>
                            <p>   {{__('profile.shop_now')}}</p>
                            <!--<label class="badge badge-warning float-right">Pharmacist</label>-->
                        </a>
                        <a class="dropdown-item" href="{{route('getBoughtsView')}}">
                            <i class="now-ui-icons users_circle-08"></i>
                            <p> {{__('profile.orders')}} </p>
                            <!--<label class="badge badge-warning float-right">Pharmacist</label>-->
                        </a> 
                        <a class="dropdown-item" href="{{route('drugStoreMapView')}}">
            ...
            <li class="nav-item dropdown">
                ...
                @endif
                @if(auth()->user()->role_id == 2)
                    <a href="{{route('getShowFavouritesView')}}"
                       class="nav-link btn-round position-relative d-inline-block">
                        <div id="cart-count" class="chat-count">{{$favourites_item_count}}</div>
                        <i class="now-ui-icons shopping_box"></i>
                    </a>
                @endif
            ...
            <li class="nav-item dropdown d-md-block d-none">
                ...
                <div class="dropdown-menu" aria-labelledby="settings">
                    ...
                    <a class="dropdown-item" href="{{route("getPostJobsView")}}">{{__('profile.jobs')}}  </a> 
                    
                    
                    @if(auth()->user()->role_id == 1) 
                    <a class="dropdown-item" href="{{route('getApproveOffersView')}}">{{__('profile.offers')}}  </a>
                    @endif
                    @if(auth()->user()->role_id == 2) 
                    <a class="dropdown-item" href="{{route('getAllUserOffersView')}}">{{__('profile.offers')}}  </a>
                    @endif
                    @if(auth()->user()->role_id == 3) 
                    <a class="dropdown-item" href="{{route('getAllUserOffersView')}}">{{__('profile.offers')}}  </a>
                    @endif
...
<nav id="menu" class="menu">
    <button class="navbar-brand bg-transparent border-0 text-danger menu__handle d-md-none" href="#">
